...
import { StyleSheet, Text, View, ScrollView, FlatList, Image, Dimensions, StatusBar, ActivityIndicator, Pressable } from 'react-native';
import { TextInput } from 'react-native-gesture-handler';
import firebase from "../firebase";
import { areaCode, sigunCode } from "../regionCode";
...
export default function HomeScreen({navigation}) {
  ...
  const filterBreweryInfo = (Code) => {
    const data = [];
    for (let i = 0; i < breweryInfo.length; i++) {
      if(breweryInfo[i]['areaCode'] === Code) {
  ...
  useEffect(() => {
    ...
    if (filteredBreweryInfo.length != 0){
      setLoading(false);
    }
  }, []);
  ...
  return (
    <View style={styles.container}>
      <View style={styles.area}>
        <TextInput style={styles.areaText} onChangeText={text => getRegion(text)}>{region}</TextInput>
      </View>
      <ScrollView>
          <View>
